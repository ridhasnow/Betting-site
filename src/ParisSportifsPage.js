...
export default function ParisSportifsPage() {
  ...
  const { addToCart, cart } = useContext(BetCartContext);

  useEffect(() => {
    getSoccerLeagues().then(setLeagues);
  }, []);
                      ...
                      {["1", "X", "2"].map(opt => (
                        <button key={opt}
                          ...
                          onClick={() => addToCart({
                            ...
                            option: opt,
                            cote: Math.floor(Math.random()*5*100)/100 + 1.5 // مؤقت: قيمة كوت عشوائية
                          })}
